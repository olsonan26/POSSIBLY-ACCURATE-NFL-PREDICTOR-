...
const WEIGHTS = [0, 0.025, 0.05, 0.075, 0.10, 0.125, 0.15, 0.20, 0.25, 0.30];
...
type Rec={id:string,season:number,p:number,y:boolean,homeChanged:boolean,awayChanged:boolean,homeQb:string,awayQb:string};
...
async function main(){
 ...
 const records:Rec[]=[];
 for(const g of parsed.filter(x=>[2024,2025,2026].includes(x.season)&&x.gameType==='REG'&&Number.isFinite(x.homeScore)&&Number.isFinite(x.awayScore)&&x.homeScore!==x.awayScore&&!(x.season===2026&&x.gameday>'2026-09-14'))){
   ...
   records.push({id:g.gameId,season:g.season,p,y:g.homeScore!>g.awayScore!,homeChanged:ch.homeChanged,awayChanged:ch.awayChanged,homeQb:r.homeQb,awayQb:r.awayQb});
 }
 const y24=records.filter(r=>r.season===2024),y25=records.filter(r=>r.season===2025),y26=records.filter(r=>r.season===2026);
 console.log(`Coverage: 2024 ${y24.length}, 2025 ${y25.length}, 2026 ${y26.length}`);
 console.log('\n2024 QB-change penalty selection (Brier)'); let best=0,bestB=Infinity;
 for(const w of WEIGHTS){const s=score(y24,w);console.log(`w=${w.toFixed(3)} ${fmt(s)}`);if(s.brier<bestB){bestB=s.brier;best=w;}}
 console.log(`SELECTED w=${best.toFixed(3)} on 2024 only`);
 console.log('\n2025 confirmation');console.log(`baseline ${fmt(score(y25,0))}`);console.log(`frozen QB continuity ${fmt(score(y25,best))}`);
 console.log('\n2026 observation through Sep 14');console.log(`baseline ${fmt(score(y26,0))}`);console.log(`frozen QB continuity ${fmt(score(y26,best))}`);
 console.log('\n2026 changed picks:');
 for(const r of y26){const p2=logistic(logit(r.p)+best*((r.awayChanged?1:0)-(r.homeChanged?1:0)));if((r.p>=.5)!==(p2>=.5))console.log(`${r.id} ${(100*r.p).toFixed(1)} -> ${(100*p2).toFixed(1)} actual=${r.y?'HOME':'AWAY'} homeChanged=${r.homeChanged} awayChanged=${r.awayChanged}`);}
}
